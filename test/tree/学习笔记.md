# Tree
- [Tree](#tree)
  - [利用栈实现二叉树前中后序遍历的更新 2023.11.5](#利用栈实现二叉树前中后序遍历的更新-2023115)
  ...
  - [31. 二叉树的直径](#31-二叉树的直径)
  - [32. 验证二叉搜索树](#32-验证二叉搜索树)
  - [33. 二叉树的右视图](#33-二叉树的右视图)
  - [34. 二叉树展开为链表](#34-二叉树展开为链表)
  - [35. 从前序与中序遍历构造二叉树](#35-从前序与中序遍历构造二叉树)

## 利用栈实现二叉树前中后序遍历的更新 2023.11.5

直接看这个人的题解[leetcode题解](https://leetcode.cn/problems/binary-tree-preorder-traversal/solutions/87526/leetcodesuan-fa-xiu-lian-dong-hua-yan-shi-xbian-2/)

因为之前写的迭代法实现前中后序遍历，前序遍历和中序遍历的写法还好，就是后序遍历的写法有些难懂，不如这个题解里清晰，我们可以直接把前序遍历改一改，因为前序遍历是根左右，我们把代码改改，改成根右左，然后把结果一反转，就是左右根，也就是后序遍历了。
...
思路：所谓的最近公共祖先问题（LCA）只有三种情况：

1. p 和 q 在root 的异侧，那么root就是p和q的最近公共祖先；
2. root 就是 p，q 在 p 的左子树或者右子树中；
3. root 就是 q，p 在 q 的左子树或者右子树中；

考虑递归，从上至下递归，直到遇到p或者q，返回，在回溯的过程中，当结点p，q 在结点root的异侧时，结点root即为最近公共祖先，向上返回root即可。

递归解析：
1. 终止条件：
    * 当越过叶结点时，则直接返回null；
    * 当root 等于p 或者 q时，代表找到了，直接返回root；
2. 递推工作：
    * 开启递归左子结点，返回值记为left；
    * 开启递归右子结点，返回值记为right；
3. 回溯：
    * 当left 和 right 同时为空：说明root的左右子树中都不包含 p 或者 q；
    * 当left 和 right 同时不为空：说明p，q分别在root的异侧，因此root为最近公共祖先，返回root；
    * 当left为空，right不为空：说明p，q都不在root的左子树中，具体又可分为两种情况：
        * p，q其中有一个在root的右子树中，此时right指向p（假设为p）
        * p，q两个都在root的右子树中，此时right指向最近公共祖先结点；
    * 当left不为空，right为空，同上。
```java
class Solution {
...
注意细节：这里计算第一层递归的时候，参数应该设置为long类型，因为targetSum有可能越界。
```java
class Solutioon{
...
这里我没用数组，直接用了一个双端队列deque，直接把结果往队列尾端加，然后在中序遍历的过程中到 k 被减小到 0 的时候打断就可以了。
...
时间复杂度：就是中序遍历的时间复杂度，所有的结点都要遍历一遍，当然这里很可能不会，但时间复杂度还是$O(N)$。
...
思路：我们要求直径，也就是树结点之间的最长路径，树结点之间的路径可以被解析为经过以某个结点为根结点的路径的结点数减一。那么我们现在找到左右子树的深度和最大的结点就好了，假如某结点左子树最大深度是L，右子树最大深度是L，然后这样L+R+1就能算出最长路径了。
...
要验证是不是二叉搜索树，要确定以下几个性质：

1. 左子树的所有结点值均小于当前结点
2. 右子树所有结点值均大于当前结点
3. 左右子树也需均为二叉搜索树

我们可以用递归来解决这个问题，用一个helper(node, lower, upper) 来表示以node为根结点，它的左右子树是否都在（lower，upper）范围内，是个开区间，如果不在，就返回false，如果在，那么我们就递归调用本方法，检查左右子树是不是分别都在正确范围内。

那么对于左右子树的递归，我们就可以写为检查左子树是否在（lower，node.val）内，右子树是否在（node.val, upper）内。
...
思路：这题的本质是求层序遍历的过程中每一层最右侧的那个结点，也就是每层最后一个结点的值。直接用层序遍历然后加到结果集中就可以。
...
思路：就直接前序遍历，然后根据前序遍历里结点的顺序，重新构造一个链表，直接让二叉树的每个结点的left = null 以及 right 等于下一个结点即可。
...
前序遍历是根左右，中序遍历是左根右。只要我们能找到根结点在两个遍历数组中的位置，那么根结点有了，对于前序遍历和中序遍历来说，根结点左子树的大小肯定是相同的，也就是表现为数组中的长度相同，那我们想办法拿到这个长度，再在这个数组长度区间内递归地构造左子树就可以了，右子树同理。

那么问题是我们如何拿到这个长度，很简单，我们用一个哈希表，映射以下中序遍历数组里每个结点及其在数组中的下标。而我们又知道前序遍历的左边界（即第一个数字）就是根结点，那么我们就能根据这个前序遍历的根结点在哈希表中定位根结点在中序遍历数组中的位置。找到这个位置后，我们直接用这个根结点下标减去中序遍历数组的左边界，就得到了长度。
...
class Solution {
    ...
    public TreeNode myBuildTree(int[] preorder, int[] inorder, int preorder_left, int preorder_right, int inorder_left, int inorder_right){
        ...
        // 前序遍历中左边界也就是第一个结点就是根结点
        int preorder_root = preorder_left;
        // 依据前序遍历中的根结点定位到中序遍历中的根结点
        ...
        // 递归地构造出左子树，并连接到根结点
        // 前序遍历中【从左边界 + 1 开始的 size_left_subtree】个元素就对应了中序遍历中【从左边界开始到inorder_root - 1】的元素
